...
contract WhitelistNFTTest is Test, BaseSetup {
    ...
    function test_MintToShouldPass() public {
        uint256 tokenId = whitelistNFTContract.mintTo(user1);
        uint256 balance = whitelistNFTContract.balanceOf(user1);
        assertEq(balance, 1);
        assertEq(tokenId, 1);
        assertEq(whitelistNFTContract.ownerOf(tokenId), user1);

        // token uri
        string memory tokenURI = whitelistNFTContract.tokenURI(tokenId);
        assertEq(tokenURI, "1");
    }
    ...
    function test_ExpectRevertMintToZeroAddress() public {
        ...
        whitelistNFTContract.mintTo(address(0));
    }
    ...
    function test_ExpectRevertTransferSoulBoundNFT() public {
        ...
        whitelistNFTContract.transferFrom(user1, user2, tokenId);
    }
    ...
    function test_BurnNFTShouldPass() public {
        ...
        whitelistNFTContract.burn(tokenId);
        balance = whitelistNFTContract.balanceOf(user1);
        assertEq(balance, 0);
        vm.stopPrank();

        vm.expectRevert(); // token is non existent hence revert
        whitelistNFTContract.ownerOf(tokenId);
    ...
    function test_ExpectRevertBurnNFT() public {
        ...
        // burn NFT
        vm.expectRevert();
        whitelistNFTContract.burn(tokenId); // burn is called by the contract owner (msg.sender)
