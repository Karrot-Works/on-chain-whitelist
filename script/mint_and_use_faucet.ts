...
const main = async () => {
    ...
    const nftContract = new Contract(deployments.earlyAccessNFT, EarlyAccessNFTArtifact.abi, sender);
    const faucetContract = new Contract(deployments.faucet, FaucetArtifact.abi, sender);
    ...
    console.log('recipient balance before:', await rpcProvider.getBalance(receiver.address));
    ...
    console.log('recipient balance after:', await rpcProvider.getBalance(receiver.address));
};
