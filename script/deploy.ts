...
const main = async () => {
  ...
  const EarlyAccessNFT = new ContractFactory(EarlyAccessNFTArtifact.abi, EarlyAccessNFTArtifact.bytecode, sender);
...
console.log('deploy Faucet contract ...');
    ...
    const Faucet = new ContractFactory(FaucetArtifact.abi, FaucetArtifact.bytecode, sender);
            // in wei, 0.001 Eth
            const claimAmount = 1000000000000000;
            // in seconds
            const cooldownDuration = 20;

    const faucetResponse = await Faucet.deploy(earlyAccessNFTAddress, claimAmount, cooldownDuration);
    ...
    writeFileSync('./data/deployments.json', JSON.stringify({
